  ...
  bluebird
    .map(prizes, (prize, index) => {
      ...
      return bluebird.all(queries).then((winners) => {
        ...
        return `<h2>${prize.tweetQuantity + prize.formQuantity}x ${
          prize.name
...
app.get("/admin/clear", (req, res) => {
  ...
  form.clear(() => {
    res.end(
      '<p>System has been cleared. <a href="/admin">Return to admin</a></p>'
    );
  });
});
...
app.get("/admin/setup", (req, res) => {
  ...
  setupPrizes().then(() => {
    res.end(
      '<p>System has been set up. <a href="/admin">Return to admin</a></p>'
    );
  });
});
